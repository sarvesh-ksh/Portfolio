    ...
    <>
      ...
      <nav
        className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${
          scrolled
            ? "bg-bg-primary/80 backdrop-blur-md border-b border-white/[0.05] shadow-xl shadow-black/30"
            : "bg-transparent"
        }`}
      >
        {/* Scroll Progress Bar */}
        <div
          className="absolute top-0 left-0 h-[2px] bg-gradient-to-r from-accent to-accent2 transition-all duration-100"
        ...
        <div className="content mx-auto px-4 sm:px-6">
          <div className="flex items-center justify-between h-16 lg:h-[70px]">
            ...
            <div className="flex items-center gap-3">
              ...
              >
                <span
                  className={`w-[18px] h-[1.5px] bg-text-primary transition-all duration-300 ${mobileOpen ? "rotate-45 translate-y-[6.5px]" : ""}`}
                />
                <span
                  className={`w-[18px] h-[1.5px] bg-text-primary transition-all duration-300 ${mobileOpen ? "opacity-0" : ""}`}
                />
                <span
                  className={`w-[18px] h-[1.5px] bg-text-primary transition-all duration-300 ${mobileOpen ? "-rotate-45 -translate-y-[6.5px]" : ""}`}
                />
      ...
      {/* ── Mobile Overlay ── */}
      <div
        className={`fixed inset-0 z-40 lg:hidden transition-all duration-400 ${mobileOpen ? "visible opacity-100" : "invisible opacity-0"}`}
      >
        {/* Backdrop */}
        <div
        ...
        {/* Drawer */}
        <div
          className={`absolute right-0 top-0 h-full w-72 bg-bg-primary/96 backdrop-blur-2xl border-l border-white/[0.06] flex flex-col pt-24 px-4 pb-8 transition-transform duration-400 ${mobileOpen ? "translate-x-0" : "translate-x-full"}`}
        >
